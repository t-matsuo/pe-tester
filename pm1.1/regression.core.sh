...
test_binary=/usr/sbin/crm_simulate
PCMK_schema_directory=/usr/share/pacemaker
...
while true ; do
	...
	-v|--valgrind)
	    ...
	    VALGRIND_CMD="valgrind -q --gen-suppressions=all --log-file=%q{valgrind_output} --show-reachable=no --leak-check=full --trace-children=no --time-stamp=yes --num-callers=20 --suppressions=/usr/share/pacemaker/tests/valgrind-pcmk.suppressions"
	    test_binary=
	    shift;;
	--valgrind-dhat)
